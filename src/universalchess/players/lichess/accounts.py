...
def lichess_account_picker_choices(
    ...
    *,
    ...
    """Rows for the lobby's account picker: Default plus every credential.

    Each item is ``(key, label, selected)``, where Default uses key ``""`` and
    means the first credential. Every account is offered: the board plays as one
...
def lichess_play_account_choices(settings, *, config=None) -> List[Tuple[str, str, bool]]:
    """Account picker rows for the lobby, marking the account it plays as."""
    from .match import lichess_account_id

    return lichess_account_picker_choices(lichess_account_id(settings), config=config)
