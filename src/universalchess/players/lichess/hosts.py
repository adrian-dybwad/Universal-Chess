...
def parse_credential_id(account_id: str) -> Tuple[str, str]:
    ...
    Canonical ids are ``{host}:{username}`` (``org:alice``). Host comes from
    that prefix or from the stored credential, never from a fallback of org.
    An empty, unprefixed, or unknown-prefix id has no host in the string.
    """
