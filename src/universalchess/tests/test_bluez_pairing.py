...
class TestGetAdapterInfo(unittest.TestCase):
    ...
    def test_reads_address_and_alias(self):
        # The happy path: Address is the MAC and Alias is the advertised friendly
        # name (preferred for name). A regression that read the wrong property (or
        # the device tree instead of the adapter) would surface the wrong MAC/name
        # on the card.
        manager = self._manager({
            "/org/bluez/hci0": {
    ...
    def test_falls_back_to_name_when_no_alias(self):
        ...
        # yield a name from the read-only Name property rather than an empty
        # string, so the card never shows a blank Bluetooth identity.
        manager = self._manager({
